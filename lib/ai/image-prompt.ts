...
export function buildImagePrompt(
  content: Pick<StoredCocktailContent, "name" | "servingGlass">,
): string {
  // English throughout: the prompt itself is English, and image providers render
  // an English drink name more reliably than a Chinese one.
  const wineName = content.name.en || content.name.cn;
  const glassType = content.servingGlass.en || "appropriate cocktail glass";
