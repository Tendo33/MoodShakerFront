...
export const PolaroidCard = forwardRef<HTMLDivElement, PolaroidCardProps>(
  ({ cocktail, imageUrl }, ref) => {
    ...
    const alcoholLevel = getLocalizedContent("alcohol_level", "english_alcohol_level");
    const glass = getLocalizedContent("serving_glass", "english_serving_glass");
    const baseSpirit = getLocalizedContent("base_spirit", "english_base_spirit");
    const flavors = language === 'en' && cocktail.english_flavor_profiles 
    ...
    const getFlavorEmoji = (flavor: string) => {
        ...
        if (lower.includes('fresh') || lower.includes('refresh') || lower.includes('清')) return '🍃';
    ...
    return (
      ...
      >
        ...
        {cocktail.english_name && (
            <div 
                className="absolute top-[20%] -right-10 rotate-90 origin-bottom-right text-[80px] font-black text-white/[0.02] whitespace-nowrap pointer-events-none font-playfair z-0"
            >
                {cocktail.english_name}
            </div>
        )}
        ...
        <div className="relative z-10 flex flex-col h-full">
            ...
            <div className="mb-8 bg-white/[0.03] border border-white/5 rounded-xl p-6 backdrop-blur-sm">
                ...
                <div className="space-y-3">
                    {ingredients.map((ing, i) => (
                        <div key={i} className="flex justify-between items-end border-b border-dashed border-white/10 pb-1 last:border-0">
                            <span className="text-sm font-medium text-white/90">
                                {language === 'en' && ing.english_name ? ing.english_name : ing.name}
                            </span>
                            <span className="text-sm font-bold text-primary/90 font-mono">
                                {language === 'en' && ing.english_amount ? ing.english_amount : ing.amount} 
                                {language === 'en' && ing.english_unit ? ing.english_unit : ing.unit}
                            </span>
                        </div>
                    ))}
                </div>
                    ...
                    {steps.map((step, i) => (
                        <div key={i} className="flex gap-4 items-baseline">
                            ...
                            <p className="text-sm text-white/80 leading-relaxed font-light">
                                {language === 'en' && step.english_description ? step.english_description : step.description}
                            </p>
                        </div>
                    ))}
                </div>
            ...
            <div className="flex items-center justify-between pt-6 border-t border-white/5 opacity-50">
                <div className="flex items-center gap-2">
                     ...
                     <span className="font-playfair font-bold text-sm tracking-wide">MoodShaker</span>
                </div>
